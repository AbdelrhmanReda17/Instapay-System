...
import Bills.Bill;
import Bills.WaterBill;

import java.io.BufferedReader;
import java.io.FileReader;

public class WaterProvider implements BillProvider{
    private final String BillFilePath = "src\\Database\\WaterBills.csv";
    @Override
    public Bill GetBill(int BillId) {
        try (BufferedReader reader = new BufferedReader(new FileReader(BillFilePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                return new WaterBill(Integer.parseInt(columns[0]),columns[1],Double.parseDouble(columns[2])
